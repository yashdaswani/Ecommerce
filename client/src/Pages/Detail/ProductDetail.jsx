...
const ProductDetail = () => {
    ...
    const addToCart = async (product) => {
        if (setProceed) {
            try {
                ...
                const { data } = await axios.post(`https://ecommerce-0gim.onrender.com/api/cart/addcart`, { _id: product._id, quantity: productQuantity }, {
                    headers: {
    ...
    const addToWhishList = async (product) => {
        if (setProceed) {
            try {
                const { data } = await axios.post(`https://ecommerce-0gim.onrender.com/api/wishlist/addwishlist`, { _id: product._id }, {
                    headers: {
                        'Authorization': authToken
                    }
                })
    ...
    const shareProduct = (product) => {
        ...
        const data = {
            text: product.name,
            title: "ecommerce",
            url: `https://ecommerce-yash.netlify.app/Detail/type/${cat}/${id}`
        }
    ...
    const getSimilarProducts = async () => {
        const { data } = await axios.post(`https://ecommerce-0gim.onrender.com/api/product/fetchproduct/type`, { userType: cat })
