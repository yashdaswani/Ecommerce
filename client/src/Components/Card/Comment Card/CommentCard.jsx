...
const CommentCard = ({ userReview, setReviews, reviews, fetchReviews }) => {
    ...
    const handleDeleteComment = async () => {
        try {
            const { data } = await axios.delete(`https://ecommerce-0gim.onrender.com/api/review/deletereview/${userReview._id}`, {
                headers: {
                    'Authorization': authToken
                }
            })
    ...
    const sendEditResponse = async () => {
        if (!editComment && !value) {
            ...
            try {
                if (authToken) {
                    const response = await axios.put(`https://ecommerce-0gim.onrender.com/api/review/editreview`,
