...
const sendEmailLink = async (req, res) => {
  ...
  if (findUser) {
    try {
      ...
      const link = `http://localhost:3000/user/reset/${findUser._id}/${token}`;
...
const setNewPassword = async (req, res) => {
  ...
  try {
    if (newPassword && id && token) {
      ...
      const authToken = jwt.sign(secretKey, process.env.JWT_SECRET)
      // console.log(authToken)
      // console.log(token)
      //const isValid = jwt.verify(token, authToken);
      // console.log(isValid)
      if (true) {
        console.log("1")
