...
const ProductReview = ({ authToken, setProceed, setOpenAlert, id }) => {
    ...
    const fetchReviews = async () => {
        ...
        const { data } = await axios.post(`https://ecommerce-0gim.onrender.com/api/review/fetchreview/${id}`, { filterType: filter })
        setReviews(data)
    }
    useEffect(() => {
        fetchReviews()
    ...
    const handleSubmitReview = async (e) => {
        ...
        if (!comment && !value) {
            ...
            try {
                if (setProceed) {
                    const { data } = await axios.post(`https://ecommerce-0gim.onrender.com/api/review/addreview`, { id: id, comment: comment, rating: value }, {
                        headers: {
