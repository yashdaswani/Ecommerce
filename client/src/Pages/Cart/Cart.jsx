...
const Cart = () => {
    ...
    const getPreviousOrder = async () => {
        const { data } = await axios.get(`https://ecommerce-0gim.onrender.com/api/getPreviousOrders`,
            {
                headers: {
                    'Authorization': authToken
                }
    ...
    const removeFromCart = async (product) => {
        if (setProceed) {
            try {
                const response = await axios.delete(`https://ecommerce-0gim.onrender.com/api/cart/deletecart/${product._id}`, {
                    headers: {
                        'Authorization': authToken
                    }
                })
