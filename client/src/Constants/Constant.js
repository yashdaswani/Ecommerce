...
const getWishList = async (setProceed, setWishlistData, authToken) => {
    if (setProceed) {
        const { data } = await axios.get(`https://ecommerce-0gim.onrender.com/api/wishlist/fetchwishlist`,
            {
                headers: {
                    'Authorization': authToken
                }
...
const getAllProducts = async (setData) => {
    try {
        const { data } = await axios.get('https://ecommerce-0gim.onrender.com/api/product/fetchproduct');
        setData(data)
...
const getSingleProduct = async (setProduct, id, setLoading) => {
    ...
    const { data } = await axios.get(`https://ecommerce-0gim.onrender.com/api/product/fetchproduct/${id}`)
    setProduct(data)
    setLoading(false);
