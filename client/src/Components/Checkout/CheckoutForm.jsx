...
const CheckoutForm = () => {
    ...
    const getUserData = async () => {
        try {
            const { data } = await axios.get(`https://ecommerce-0gim.onrender.com/api/auth/getuser`, {
                headers: {
    ...
    const checkOutHandler = async (e) => {
        ...
        if (!userDetails.firstName || !userDetails.lastName || !userDetails.userEmail || !userDetails.phoneNumber || !userDetails.address || !userDetails.zipCode || !userDetails.city || !userDetails.userState) {
            ...
            try {
                const { data: { key } } = await axios.get(`https://ecommerce-0gim.onrender.com/api/getkey`)
                const { data } = await axios.post(`https://ecommerce-0gim.onrender.com/api/checkout`, {
                    amount: totalAmount,
                ...
                const options = {
                    ...
                    callback_url: "https://ecommerce-0gim.onrender.com/api/paymentverification",
                    prefill: {
                        name: userData.firstName + ' ' + userData.lastName,
